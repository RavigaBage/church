...
class UploadData
{
    private $target_dir = "../images/calenda/";
    public function _log($str)
    {   
        $log_str = date('d.m.Y') . ": {$str}\r\n";
        if($str == 'success'){
            return json_encode(["status"=>"success","name"=>$this->target_dir . '/' . $_POST['resumableFilename']]);
        }else{
    ...
    public function file_registry(){
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (file_exists($chunk_file)) {
                ...
                return json_encode(["status"=>"error","name"=>"not found"]);
