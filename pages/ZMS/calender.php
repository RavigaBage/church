...
if (isset($_SESSION['unique_id'])) {
    ...
    if ((hash_equals($known, $token))) {
        if (!isset($_SESSION['calenda_Log'])) {
            $date = date('Y-m-d H:i:s');
            $newquest = $newDataRequest->DataHistory($unique_id, "Admin permit was used to logged in", $date, "Dashboard calender", "Admin permit was used logged in to dashboard");
            $decode = json_decode($newquest);
            if ($decode == 'Success') {
                $_SESSION['calenda_Log'] = true;
                $condition = true;
            }
        } else {
            $condition = true;
