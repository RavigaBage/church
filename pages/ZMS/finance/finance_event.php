...
if (isset($_SESSION['unique_id'])) {
    ...
    if ((hash_equals($known, $token))) {
        if (!isset($_SESSION['Finance_event_Log'])) {
            $date = date('Y-m-d H:i:s');
            $newquest = $newDataRequest->DataHistory($unique_id, "Admin permit was used to logged in", $date, "Dashboard finance event", "Admin permit was used logged in to dashboard");
            $decode = json_decode($newquest);
            if ($decode == 'Success') {
                $_SESSION['Finance_event_Log'] = true;
                $condition = true;
            }
        } else {
            $condition = true;
                                ...
                                <div class="content_page_event">
                                    ...
                                    <div class="records_table">
                                        ...
                                        <tbody>';
                                                ...
                                                echo "<header class='danger' style='font-size:25px;'>" . $update . "</header>";
                                            }
        ...
        <script>
            ...
            function UpdateItemFunction(value, ActivityMenu) {
                ...
                document.querySelector(
                    '.event_menu_add.form_data select[name="name"]'
                ).value = newObject["id"];
                document.querySelector(
                    '.event_menu_add.form_data input[name="amount"]'
                ).value = newObject["amount"];
                document.querySelector(
                    '.event_menu_add.form_data input[name="Date"]'
                ).value = newObject["date"];
                document.querySelector(
                    '.event_menu_add.form_data input[name="delete_key"]'
                ).value = newObject["id"];
                document.querySelector(
                    '.event_menu_add.form_data input[name="formName"]'
                ).value = newObject["Formname"];
                document.querySelector(
                    '.event_menu_add.form_data select[name="medium"]'
